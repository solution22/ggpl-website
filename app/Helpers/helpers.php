...
function getBalance($modelObject, $attributeName = 'balance')
{
    if ($modelObject[$attributeName] != null && strlen($modelObject[$attributeName]) > 0) {
        $modelObject[$attributeName] = number_format((float)$modelObject[$attributeName], 2, '.', '');
        if($modelObject[$attributeName] < 0) {
            $icon = '<i class="fa fa-arrow-down text-success"></i>';
        } else {
            $icon = '<i class="fa fa-arrow-up text-danger"></i>'; 
        }
        if (setting('currency_right', false) != false) {
            return $icon." ".abs($modelObject[$attributeName]) . "<span>" . setting('default_currency') . "</span>";
        } else {
            return $icon." "."<span>" . setting('default_currency') . "</span>" . abs($modelObject[$attributeName]);
        }
    }
    return '-';
}

function getDue($date_1, $date_2)
{
...
function amountInWords($number = 0) {
    $no = floor($number);
    ...
    return $net_pay_per_annum_words;
}

function translateWord($name = '', $language = '') {
      $url="http://api.mymemory.translated.net/get?q=".$name."&langpair=en|".$language;
      $result = file_get_contents($url);
      $obj=json_decode($result);
      $obj=$obj->{'responseData'};
      $trans_word = $obj->{'translatedText'};
      return $trans_word;
}

function autoIncrementId($table = '') {
    $databaseName = Config::get('database.connections.'.Config::get('database.default'));
    $next_id  = DB::select(DB::raw("SELECT AUTO_INCREMENT as id
                    FROM information_schema.tables
                    WHERE table_name = '$table'
                    AND table_schema = '".$databaseName['database']."'
                "))[0]->id;
    return $next_id;            
}
