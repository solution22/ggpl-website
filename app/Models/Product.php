...
class Product extends Model implements HasMedia
{
     ...
     * @return \Illuminate\Database\Eloquent\Relations\HasMany
     **/
    public function productReviewsSum()
    {
        $totalRate   =  $this->hasMany(\App\Models\ProductReview::class, 'product_id')->sum('rate');
        $totalReview =  $this->hasMany(\App\Models\ProductReview::class, 'product_id')->count();
        if($totalReview > 0 && $totalRate > 0) {
            return  round($totalRate / $totalReview);
        } else {
            return  5;
        }
    }
    ...
    {
        return $this->belongsTo(\App\Models\Favorite::class, 'product_id', 'id')->where('user_id',auth()->user()->id);
    }
